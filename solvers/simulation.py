...
from utils import generate_wrapper, wait_for_file
import subprocess
import os
import warnings
import yt
...
class Simulation:
    def __init__(self, u_init_fn: str, base_dir: str,
                 exec_name: str, exec_args: list, exec_nproc: int, exec_finish_check_fn: str,
        ...
        :param exec_name: The executable name of the solver
        :param exec_args: The arguments of the executable
        :param exec_nproc: The number of processors to run the executable
        :param exec_finish_check_fn: The file name to check whether the simulation is finished
        ...
        if self.restart:
            ...
            self.exec_nproc = exec_nproc
            self.exec_finish_check_fn = exec_finish_check_fn
            ...
            if self.rank == 0:
                ...
                self.run_simulation_with_wrapper(self.exec_name, self.exec_nproc, self.exec_args,
                                                 self.exec_finish_check_fn)
    ...
    def run_simulation_with_wrapper(self, exec_cmd: str, exec_nproc: int, exec_args: list = None,
                                    exec_finish_check_fn: str = None):
        generate_wrapper(exec_cmd, wrapper_path=self.base_dir)
        # Now start the simulation
        current_dir = os.getcwd()
        os.chdir(self.base_dir)
        new_comm = self.comm_self.Spawn(command='python3', args=['wrapper.py'] + exec_args, maxprocs=exec_nproc)
        if exec_finish_check_fn is None:
            warnings.warn("The check file name is not specified! The simulation will not be waited to finish."
                          "Please make sure the simulation is finished before proceeding."
                          "One possible value can be stdout.txt.")
        else:
            wait_for_file(exec_finish_check_fn, timeout=60 * 60 * 24 * 7, poll_interval=5)
    ...
    def generate_u_pert(self, pert_mag):
        ...
        if self.rank == 0:
            ...
            del ds
            self.comm.bcast(u_pert, root=0)
        else:
            u_pert = None
            u_pert = self.comm.bcast(u_pert, root=0)
        return u_pert

    def proceed_simulation(self, params, t1,
                           exec_name=None, exec_args=None, exec_nproc=None,
