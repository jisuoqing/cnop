...
from utils import generate_shell_wrapper, wait_for_file, wait_for_last_line
import subprocess
import os
import warnings
import yt
...
class Simulation:
    def __init__(self, u_init_fn: str, base_dir: str,
                 ...
                 wrapper_name: str, wrapper_output: str,
                 wrapper_start_check_timeout: float, wrapper_finish_check_timeout: float,
                 wrapper_check_poll_interval: float, wrapper_successful_check_fn: str,
        ...
        :param wrapper_name: The name of the shell wrapper, e.g. "wrapper.sh"
        :param wrapper_output: The name of the output file of the shell wrapper, e.g. "stdout.txt"
        :param wrapper_start_check_timeout: The timeout to check the simulation starts running, should be short
        :param wrapper_finish_check_timeout: The timeout to check the simulation is finished, can be long
        :param wrapper_check_poll_interval: The interval to poll the simulation status
        :param wrapper_successful_check_fn: The file should be created by the end of the wrapper if successful
        ...
        if self.restart:
            ...
            self.wrapper_output = wrapper_output
            self.wrapper_running_check_timeout = wrapper_start_check_timeout
            ...
            self.wrapper_successful_check_fn = wrapper_successful_check_fn
    ...
    def run_simulation_with_shell_wrapper(self):
        # First, delete the wrapper output to avoid confusion from wait_for_file and wait_for_last_line
        if os.path.exists(f"{self.base_dir}/{self.wrapper_output}"):
            os.remove(f"{self.base_dir}/{self.wrapper_output}")
        if self.wrapper_successful_check_fn is not None:
            if os.path.exists(f"{self.base_dir}/{self.wrapper_successful_check_fn}"):
                os.remove(f"{self.base_dir}/{self.wrapper_successful_check_fn}")
                logging.debug(f"The finish check file {self.wrapper_successful_check_fn} is deleted!")

        ending_remark = f"Simulation finished at {self.mpi_rank} on {self.mpi.Get_processor_name()}"
        ...
        info.Free()
        if not wait_for_file(f"{self.base_dir}/{self.wrapper_output}",
            ...
            raise RuntimeError("The simulation is not running since "
                               f"{self.base_dir}/{self.wrapper_output} is not generated!")

        if not wait_for_last_line(f"{self.base_dir}/{self.wrapper_output}", ending_remark,
                                  timeout=self.wrapper_finish_check_timeout,
                                  poll_interval=self.wrapper_check_poll_interval):
            raise RuntimeError(
                f"The simulation is not finished within {self.wrapper_finish_check_timeout} seconds!")
        child_comm.Free()
    ...
    def proceed_simulation(self, params, t1,
                           ...
                           wrapper_name=None, wrapper_output=None,
                           wrapper_running_check_timeout=None, wrapper_finish_check_timeout=None,
                           wrapper_check_poll_interval=None, wrapper_successful_check_fn=None,
        ...
        if wrapper_check_poll_interval is not None:
            self.wrapper_check_poll_interval = wrapper_check_poll_interval
        # do not inherit wrapper_successful_check_fn for proceeding simulation since a different file is supposed to
        # be created
        self.wrapper_successful_check_fn = wrapper_successful_check_fn
